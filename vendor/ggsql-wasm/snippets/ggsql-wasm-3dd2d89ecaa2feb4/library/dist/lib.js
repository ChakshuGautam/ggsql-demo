...
function inferCSVColumnType(values) {
  let allInt = true;
  let allNum = true;
  let allBool = true;
...
function buildCSVColumn(name, rawStrings, colType) {
  const len = rawStrings.length;
  const nulls = new Uint8Array(len);
  if (colType === "f64" || colType === "i64") {
    const values2 = new Float64Array(len);
    for (let i = 0; i < len; i++) {
      if (rawStrings[i] === null) {
        values2[i] = 0;
        nulls[i] = 0;
      } else {
        values2[i] = Number(rawStrings[i]);
        nulls[i] = 1;
      }
    }
    return { name, type: colType, values: values2, nulls };
  }
  if (colType === "bool") {
    const values2 = new Uint8Array(len);
    for (let i = 0; i < len; i++) {
      if (rawStrings[i] === null) {
        values2[i] = 0;
        nulls[i] = 0;
      } else {
        values2[i] = rawStrings[i].toLowerCase() === "true" ? 1 : 0;
        nulls[i] = 1;
      }
    }
    return { name, type: colType, values: values2, nulls };
...
async function parquetRead(options) {
  options.metadata ?? (options.metadata = await parquetMetadataAsync(options.file, options));
...
// src/parquet.ts
async function convert_parquet(bytes) {
...
function buildColumn(name, rawValues) {
  ...
  if (type === "f64" || type === "i64") {
    ...
    return { name, type, values: values2, nulls };
  }
  if (type === "bool") {
    const values2 = new Uint8Array(len);
    for (let i = 0; i < len; i++) {
  ...
  if (type === "date") {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (v === null || v === void 0) {
        ...
        values2[i] = Math.floor((v.getTime() - EPOCH) / MS_PER_DAY);
        nulls[i] = 1;
      }
    }
    return { name, type, values: values2, nulls };
